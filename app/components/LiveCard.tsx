...
export default function LiveCard({ title, intervenant, image }: LiveCardProps) {
    return (
        <div className="flex flex-col bg-card-bg rounded-[20px] overflow-hidden w-[350px] md:w-[400px] lg:w-[420px] flex-shrink-0 snap-center group cursor-pointer border border-pure-white/5 hover:border-pure-white/10 transition-all duration-300 shadow-xl">
            {/* Image section */}
            <div className="relative w-full h-[220px] md:h-[250px] lg:h-[270px] overflow-hidden">
                <Image
                    src={image}
                    alt={title}
                    fill
            ...
            <div className="flex flex-col flex-grow p-6 lg:p-8">
                <h4 className="text-pure-white font-semibold text-[18px] lg:text-[20px] leading-snug mb-6 group-hover:text-light-orange transition-colors duration-300">
                    {title}
                </h4>
